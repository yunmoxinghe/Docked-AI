...
namespace Docked_AI.Features.MainWindowContent.ContentArea
{
    ...
    public class NavigationService
    {
        ...
        public bool GoBack()
        {
            ...
            // ⭐ 检查是否使用自定义缓存（仅 WebBrowserPage）
            if (!string.IsNullOrEmpty(cacheKey) && _cacheManager.IsPageCached(cacheKey))
            {
                System.Diagnostics.Debug.WriteLine($"[NavigationService] 使用自定义缓存返回: {cacheKey}");
                ...
                if (cachedPage == null)
                {
                    System.Diagnostics.Debug.WriteLine($"[NavigationService] ⚠️ 缓存页面不存在，回退到 Frame.GoBack()");
                    _frame.BackStack.Add(backEntry); // 恢复 BackStack
                    _frame.GoBack();
                    return true;
                }

                // 直接设置内容（返回到缓存页时不走 Frame.GoBack，避免触发前进方向的内容动画）
            ...
            {
                // ⭐ 其他页面（管理页等）使用 Frame 原生缓存（NavigationCacheMode）
                System.Diagnostics.Debug.WriteLine($"[NavigationService] 页面未使用自定义缓存，使用 Frame.GoBack()");
                _frame.GoBack();
                return true;
            }
        }
        ...
        {
            // ⭐ WebBrowserPage 使用自定义 LRU 缓存（基于 shortcut.Id）
            if (pageType.Name == "WebBrowserPage" && parameter is Pages.WebApp.Shared.WebAppShortcut shortcut)
            {
                return $"WebBrowser_{shortcut.Id}";
            }

            // ⭐ 其他页面返回 null，使用 WinUI 原生缓存（NavigationCacheMode）
            // WebAppManagementPage 和 WebAppDetailPage 可以在 XAML 中设置 NavigationCacheMode="Required"
            // 由 Frame 自己管理缓存，不计入自定义缓存池
            return null;
        }
